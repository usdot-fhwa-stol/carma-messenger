...
CarmaJS.WidgetFramework.truckInspection = (function () {
    ...
    var CreateTruckInspectionSafetyLogPerVINWidget = function(container){
            ...
            var CarrierFrame = createDivFramePanelText('CarrierFrameId', 'CarrierFrame', 'CarrierPanelId', 'CarrierPanel', 'USDOT #', 'CarrierTextId', 'CarrierText','--');
    ...
    var IsValidVINNumber = function(vin_number)
    {
        ...
        valid_truck_vin_numbers.forEach((valid_vin)=>{
            if(valid_vin == vin_number){
                console.log("Found Valid VIN:"+vin_number);
                isValid = true;
            }               
        });
        if(!isValid)
            console.log("Found Invalid VIN:"+vin_number);
