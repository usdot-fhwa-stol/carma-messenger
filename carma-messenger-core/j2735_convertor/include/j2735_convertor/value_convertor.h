...
public:
   ...
   * @param unavailability_value The value of the input which would mean that value was not representative of actual
   * data
   *
   * @return The converted input value in the new output unit and type
   */
  template <typename T, typename U, typename V, typename W, typename X>
  static T valueJ2735ToCav(const U in, const double conversion_factor, V& presence_vector, const W presence_flag,
                           const X unavailability_value)
  {
    if (in != (U)unavailability_value)
   ...
   * @param presence_flag A flag with a single bit set to mark the presence vector as needed
   * @param unavailability_value The value of the output which would mean that value was not representative of actual
   * data
   *
   * @return The converted input value in the new output unit and type
   */
  template <typename T, typename U, typename V, typename W, typename X>
  static T valueCavToJ2735(const U in, const double conversion_factor, const V presence_vector, const W presence_flag,
                           const X unavailability_value)
  {
    if (presence_vector & (V)presence_flag)
