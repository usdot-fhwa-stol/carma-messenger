...
namespace cpp_message
{
    ...
    boost::optional<carma_v2x_msgs::msg::EmergencyVehicleResponse> Emergency_Vehicle_Response::decode_emergency_vehicle_response_message(std::vector<uint8_t> &binary_array)
    {
        ...
        if (rval.code == RC_OK)
        {
            ...
            if (str_len >= Header_constant.STATIC_ID_MIN_LENGTH && str_len <= Header_constant.STATIC_ID_MAX_LENGTH)
            {
                ...
                {
                    sender_id += message->value.choice.TestMessage06.header.hostStaticId.buf[i];
                }
            }
            else
            ...
            // get recepient id
            str_len = message->value.choice.TestMessage06.header.targetStaticId.size;
            if (str_len >= Header_constant.STATIC_ID_MIN_LENGTH && str_len <= Header_constant.STATIC_ID_MAX_LENGTH)
            {
                for (size_t i = 0; i < str_len; i++)
                {
                    recipient_id += message->value.choice.TestMessage06.header.targetStaticId.buf[i];
                }
            }
            else
            ...
            str_len = message->value.choice.TestMessage06.header.hostBSMId.size;
            for (size_t i = 0; i < str_len; i++)
            {
                sender_bsm_id += message->value.choice.TestMessage06.header.hostBSMId.buf[i];
            }
            ...
            // get plan id
            str_len = message->value.choice.TestMessage06.header.planId.size;
            if (str_len == Header_constant.GUID_LENGTH)
            {
                for (size_t i = 0; i < str_len; i++)
                {
                    plan_id += message->value.choice.TestMessage06.header.planId.buf[i];
                }
            }
            else
            ...
            if (message->value.choice.TestMessage06.body.reason)
            {
                ...
                if (str_len >= REASON_MIN_LENGTH && str_len <= REASON_MAX_LENGTH)
                {
                    for (size_t i = 0; i < str_len; i++)
                    {
                        reason += message->value.choice.TestMessage06.body.reason->buf[i];
                    }
    ...
    boost::optional<std::vector<uint8_t>> Emergency_Vehicle_Response::encode_emergency_vehicle_response_message(carma_v2x_msgs::msg::EmergencyVehicleResponse plainMessage)
    {
        ...
        message->value.choice.TestMessage06.header.hostStaticId.buf = string_content_hostId;
        message->value.choice.TestMessage06.header.hostStaticId.size = string_size;
        ...
        message->value.choice.TestMessage06.header.targetStaticId.buf = string_content_targetId;
        message->value.choice.TestMessage06.header.targetStaticId.size = string_size;
        ...
        message->value.choice.TestMessage06.header.hostBSMId.buf = string_content_BSMId;
        message->value.choice.TestMessage06.header.hostBSMId.size = string_size;
        ...
        message->value.choice.TestMessage06.header.planId.buf = string_content_planId;
        message->value.choice.TestMessage06.header.planId.size = string_size;
        ...
        message->value.choice.TestMessage06.header.timestamp.buf = string_content_timestamp;
        message->value.choice.TestMessage06.header.timestamp.size = Header.TIMESTAMP_MESSAGE_LENGTH;
